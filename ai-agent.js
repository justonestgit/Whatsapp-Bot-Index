...
function criarAgenteIA(dependencias) {
    ...
    async function responder(pergunta, contexto) {
        ...
        const system = [
            ...
            'Apresente o resultado das ferramentas de forma natural, sem dizer que consultou uma ferramenta.'
        ].join(' ');
        ...
        let input = [{
            role: 'user',
            content: `Contexto seguro da mensagem: ${contextoTexto}\n\nPergunta do usuário: ${String(pergunta || '').trim()}`
        }];
        ...
        for (let rodada = 0; rodada < MAX_TOOL_ROUNDS; rodada++) {
            ...
            if (!chamadas.length) {
                return {
                    sucesso: true,
                    texto: String(response.output_text || '').trim() || 'Não consegui formular uma resposta agora.'
                };
            }
            ...
            for (const chamada of chamadas) {
                ...
                let resultado;
                try {
                    resultado = await executarTool(chamada.name, argumentos, contexto);
                } catch (erro) {
