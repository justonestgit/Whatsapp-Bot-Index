...
const ultimoSimih = new Map();

function ehMensagemDeComando(message) {
    const corpo = String(message?.body || '').trim();
    if (!corpo || !message?.from?.endsWith('@g.us')) return false;
    const prefixo = obterPrefixoGrupo(message.from);
    const config = obterConfigAdmin(message.from);
    return corpo.startsWith(prefixo) || (config?.multiprefix && corpo.startsWith(PREFIXO));
}

function mensagemMencionaBot(message) {
    const idBot = client.info?.wid?._serialized || client.info?.wid?.user || '';
    const ids = Array.isArray(message?.mentionedIds) ? message.mentionedIds : [];
...
async function processarSimih(message) {
    if (!message?.from?.endsWith('@g.us')) return false;
    const config = obterConfigAdmin(message.from);
    if (!config?.simih && !config?.simih2) return false;
    if (!process.env.OPENAI_API_KEY) return false;
    if (ehMensagemDeComando(message)) return false;
    const texto = String(message.body || '').trim();
    if (!texto) return false;

    const deveResponder = Boolean(config.simih2) || Boolean(config.simih && (mensagemMencionaBot(message) || message.hasQuotedMsg));
    if (!deveResponder) return false;
...
async function verificarModoEspecial(message, comando) {
    ...
    if (config?.modoRPG && !COMANDOS_RPG.has(comando) && !['config','modorpg','modogamer','simih','simih2','autodl','menu','rpg'].includes(comando)) {
        await responderCitando(message, '⚔️ _Este grupo está no modo RPG. Apenas comandos do RPG estão liberados._');
        return false;
    }
    if (config?.modoGamer && !COMANDOS_GAMER.has(comando) && !['config','modorpg','modogamer','simih','simih2','autodl','menu','jogos'].includes(comando)) {
        await responderCitando(message, '🎮 _Este grupo está no modo Gamer. Apenas comandos de jogos estão liberados._');
        return false;
    }
    return true;
}

async function comandoModoEspecial(message, argumentos, campo, titulo, emoji) {
    if (!(await exigirAdmin(message))) return;
    const config = obterConfigAdmin(message.from);
    const valor = String(argumentos || '').trim().toLowerCase();
    if (!['on','off','ativar','desativar','ativo','inativo','sim','não','nao'].includes(valor)) {
        await responderCitando(message, `${emoji} *${titulo}*\n\nEstado: *${config[campo] ? 'ATIVO' : 'INATIVO'}*\n\nUse *${obterPrefixoGrupo(message.from)}${campo.toLowerCase()} on/off*.`);
        return;
    }
    const ligado = ['on','ativar','ativo','sim'].includes(valor);
    config[campo] = ligado;
    salvarConfigAdmin();
    await reagir(message, ligado ? '✅' : '❌');
    await responderCitando(message, `${emoji} *${titulo} ${ligado ? 'ATIVADO' : 'DESATIVADO'}.*`);
}

function extrairUrlSuportada(texto) {
    const match = String(texto || '').match(/https?:\/\/[^\s<>]+/i);
    if (!match) return null;
    const url = match[0].replace(/[),.!?]+$/, '');
    try {
        const host = new URL(url).hostname.toLowerCase();
        if (host === 'youtube.com' || host.endsWith('.youtube.com') || host === 'youtu.be' || host === 'www.youtube-nocookie.com') return url;
    } catch {}
    return null;
}

async function processarAutodl(message) {
    if (!message?.from?.endsWith('@g.us')) return false;
    const config = obterConfigAdmin(message.from);
    if (!config?.autodl || ehMensagemDeComando(message)) return false;
    const url = extrairUrlSuportada(message.body);
    if (!url) return false;
    try {
        await tocarVideo(message, url);
        return true;
    } catch (erro) {
        console.error('⚠️ Erro no Autodl:', erro.message);
        return false;
    }
}

async function comandoAutodl(message, argumentos) {
    if (!(await exigirAdmin(message))) return;
    const config = obterConfigAdmin(message.from);
    const valor = String(argumentos || '').trim().toLowerCase();
    if (!['on','off','ativar','desativar','ativo','inativo','sim','não','nao'].includes(valor)) {
        await responderCitando(message, `⬇️ *AUTODL*\n\nEstado: *${config.autodl ? 'ATIVO' : 'INATIVO'}*\n\nUse *${obterPrefixoGrupo(message.from)}autodl on/off*.`);
        return;
    }
    const ligado = ['on','ativar','ativo','sim'].includes(valor);
    config.autodl = ligado;
    salvarConfigAdmin();
    await reagir(message, ligado ? '✅' : '❌');
    await responderCitando(message, `⬇️ *AUTODL ${ligado ? 'ATIVADO' : 'DESATIVADO'}.*\n\n_Quando ativo, links do YouTube enviados no grupo serão baixados automaticamente._`);
}
...
async function processarComando(
    ...
    if (!(await verificarModoEspecial(message, comando))) return;
...
case 'filhosranking':
    ...
    break;
        ...
        case 'simih':
            await comandoSimih(message, argumentos, 'simih'); break;
        case 'simih2':
            await comandoSimih(message, argumentos, 'simih2'); break;
        case 'modorpg':
        case 'modo-rpg':
            await comandoModoEspecial(message, argumentos, 'modoRPG', 'MODO RPG', '⚔️'); break;
        case 'modogamer':
        case 'modo-gamer':
            await comandoModoEspecial(message, argumentos, 'modoGamer', 'MODO GAMER', '🎮'); break;
        case 'autodl':
            await comandoAutodl(message, argumentos); break;
        case 'config':
        case 'conf':
...
// 🤖 AUTORESPONDER
            try { await processarSimih(message); } catch (erroSimih) { console.error('⚠️ Erro no Simih:', erroSimih.message); }
            try { await processarAutodl(message); } catch (erroAutodl) { console.error('⚠️ Erro no Autodl:', erroAutodl.message); }
